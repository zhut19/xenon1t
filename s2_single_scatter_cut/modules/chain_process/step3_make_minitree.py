...
class OtherLargeS2s(hax.minitrees.TreeMaker):
    """Information on the large s2 peaks other than the interaction peak. (e.g. first_largest_s2_area)
    Provides:
     - x_largest_s2_y

    x denotes the order of the area of the peak:
     - first:  The 1st largest among peaks other than main interaction peak
     - second: The 2nd largest among peaks other than main interaction peak
     - third:  The 3rd largest among peaks other than main interaction peak
    ...
    peak_name = [ order + '_largest_s2_' for order in ['first','second','third']]
    peak_fields = ['area', 'range_50p_area', 'area_fraction_top', 'x', 'y', 'z', 
                   'corrected_area', 'delay_is1', 'delay_is2']
    __version__ = '0.0.2'
    ...
    def extract_data(self, event):
        ...
        if len(event.interactions) != 0:
            ...
            other_s2s = [ix for ix in event.s2s if ix != interaction.s2] # To match largest_other_s2
